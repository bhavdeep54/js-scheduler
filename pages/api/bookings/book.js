...
export default async function handler(req, res) {
  ...
  // ✅ only buyers can book
  if (sess.user.role !== "buyer") {
    return res.status(403).json({ error: "Only buyers can book" });
  }
  ...
  if (!sellerId || !start || !end) {
    return res
      .status(400)
      .json({ error: "sellerId, start, and end are required" });
  }
  ...
  const buyerTokens = await Seller.findOne({ user: sess.user._id }).lean();

  if (!sellerTokens || !sellerTokens.encryptedRefreshToken) {
    return res.status(400).json({ error: "Seller has not connected calendar" });
  }
  ...
  try {
    ...
    // create a mirror event on buyer's calendar (if token exists)
      ...
      try {
        ...
        await buyerCal.events.insert({
          ...
          requestBody: {
            ...
            attendees: [{ email: sellerUser.email }, { email: sess.user.email }],
            description: `Booked via Next.js Scheduler. Join: ${
              meetingLink || ""
            }`,
          },
          sendUpdates: "all",
    ...
    return res.json({ ok: true, event: createdEvent, meetingLink });
  } catch (err) {
